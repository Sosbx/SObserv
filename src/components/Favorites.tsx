...
export const Favorites: React.FC<FavoritesProps> = ({ onReportChange }) => {
  ...
  const handleFavoriteClick = async (favorite: any) => {
    if (!isEditMode) {
      try {
        await navigator.clipboard.writeText(favorite.content);
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
          <h2 className="text-lg font-semibold text-primary-900">Mes Favoris</h2>
          <select
            value={selectedCategory}
            onChange={(e) => setSelectedCategory(e.target.value)}
            className="w-full sm:w-auto px-3 py-1.5 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
          >
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          >
            ...
            <span className="whitespace-nowrap">{isEditMode ? 'Terminer' : 'Modifier'}</span>
          </button>
          <button
            onClick={handleAddNew}
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors shadow-sm"
          >
            <Plus className="h-4 w-4" />
            <span className="whitespace-nowrap">Nouveau favori</span>
          </button>
        </div>
      </div>
        ...
        {filteredFavorites.map((favorite) => (
          ...
          >
            ...
            >
              ...
              <span className={`text-xs ${CATEGORIES[favorite.category as keyof typeof CATEGORIES]?.color || 'text-gray-500'}`}>
                {CATEGORIES[favorite.category as keyof typeof CATEGORIES]?.label || 'Autre'}
              </span>
            </button>
